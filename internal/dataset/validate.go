...
func ValidateLabels(root string) (ValidationReport, error) {
	...
	for _, image := range images {
		...
		boxes, errors, detectionAnnotations, segmentationAnnotations := validateOneLabel(labelPath, classes)
		report.DetectionAnnotations += detectionAnnotations
		report.SegmentationAnnotations += segmentationAnnotations
		if len(errors) > 0 {
...
func validateOneLabel(path string, classes []string) ([]yoloBox, []string, int, int) {
	file, err := os.Open(path)
	if err != nil {
		return nil, []string{err.Error()}, 0, 0
	}
	defer file.Close()

	var boxes []yoloBox
	var problems []string
	detectionAnnotations := 0
	segmentationAnnotations := 0
	lineNo := 0
	...
	for scanner.Scan() {
		...
		isDetection := len(parts) == 5
		isSegmentation := len(parts) >= 7 && len(parts)%2 == 1
		if !isDetection && !isSegmentation {
			problems = append(problems, fmt.Sprintf("第 %d 行：应为 5 列检测框，或类别加至少 3 个分割点，实际有 %d 列", lineNo, len(parts)))
			continue
		}
		...
		if isDetection {
			...
			for _, value := range values {
				if value < 0 || value > 1 {
					problems = append(problems, fmt.Sprintf("第 %d 行：分割点坐标必须在 0 到 1 之间", lineNo))
					break
				}
			}
		}
	...
	return boxes, problems, detectionAnnotations, segmentationAnnotations
}
